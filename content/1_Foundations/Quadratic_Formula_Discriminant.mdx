...
Quick! Find all zero's of this quadratic:
...
The quadratic formula is a relativeky short formula that gives us all roots of a second-degree polynomial (or *quadratic*). Namely, for a quadratic: $ax^2 + bx + c$, its roots are:
...
- if a problem notes a quadratic has two real soluions, that implies the discriminant is a perfect square. Hence, setting $b^2 - 4ac$ to $c^2$ for some integer c may result in useful deductions.
- Problems may explicitly state whether the quadratic has two real solutions, one real solutions, or no real solutions which tells us the negativity of the discriminant.
