...
Exponent Rules are used in almost every problem involving some form of algebra and are extremely important to know to progress further.  This module will cover the fundamentals of exponentiation, exponent rules, and common forms fo exponentiation that appear frequently.
...
- **Exponent**: How many times the number is being repeatedly multplied. In the expression: $5 \cdot 5 \cdot 5$, the exponent is $3$ as there are $3$ instances of $5$ being multplied.
...
A negative exponent means *reciprocal* — not a negative result:
...
$$a^{1/n} = \sqrt[n]{a} \qquad \text{and more generally,} \qquad a^{m/n} = \left(\sqrt[n]{a}\right)^m$$

*Example:* $8^{1/3} = \sqrt[3]{8} = 2$ and $8^{2/3} = \left(\sqrt[3]{8}\right)^2 = 4$
